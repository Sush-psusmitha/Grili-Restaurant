...
*/
// hide a preloader screen once the page has fully loaded
...
const sliderNext = function(){
    if(currentSliderPos >= heroSliderItems.length - 1){
        currentSliderPos = 0; // If at last slide, go back to first slide
    }else{
        currentSliderPos++; // Otherwise, move to the next slide
    }
    updateSliderPos(); // Call function to update slide position
}
...
addEventOnElements([heroSliderNextBtn,heroSliderPrevBtn],"mouseover", function(){clearInterval(autoSliderInterval);})

addEventOnElements([heroSliderNextBtn,heroSliderPrevBtn],"mouseout",autoSlider); 
